...
module simple_dual_port_ram_single_clk_i
#(
...
(
    ...
    input       [(ADDR_WIDTH):0]  Read_address_i, 
    input       [(DATA_WIDTH-1):0]  data_input___i,
    output  reg [(DATA_WIDTH-1):0]  data_output__o
);

    reg [(DATA_WIDTH-1):0] RAM_Structure [2303:0]; //2**(ADDR_WIDTH+1)
	 initial begin  //load hexadecimal data in txt
		$readmemb("R.txt", RAM_Structure);		
		end
    //C:\Dani\Doctorado\R_INV\output_files\
